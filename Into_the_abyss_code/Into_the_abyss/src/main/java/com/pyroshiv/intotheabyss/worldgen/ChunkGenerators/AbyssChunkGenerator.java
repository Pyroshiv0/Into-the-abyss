package com.pyroshiv.intotheabyss.worldgen.ChunkGenerators;
...
public class AbyssChunkGenerator extends ChunkGenerator {
    ...
    public static final MapCodec<AbyssChunkGenerator> ABYSSCODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            // add your fields here, e.g. a biome source and noise settings
            AbyssBiomeSource.CODEC.fieldOf("biome_source").forGetter(cg -> (AbyssBiomeSource) cg.getBiomeSource()),
            Codec.INT.fieldOf("sea_level").forGetter(AbyssChunkGenerator::getSeaLevel),
            Codec.INT.fieldOf("world_height").forGetter(AbyssChunkGenerator::getWorldHeight)
    ).apply(instance, AbyssChunkGenerator::new));



    public static final Codec<AbyssChunkGenerator> FULL_CODEC = ABYSSCODEC.codec();
    private final Integer seaLevel;
    private final Integer worldHeight;

    public AbyssChunkGenerator(AbyssBiomeSource biomeSource, Integer seaLevel, Integer worldHeight){
        super(biomeSource);
    ...
    private Integer getWorldHeight() {return this.worldHeight;}
}
